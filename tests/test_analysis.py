# ==========================================
# 🧪 TEST ANALYSIS COMPOUND WITH CAPSYS
# ==========================================
...
# ==========================================
# Format : ( "Formule", [ "Lignes de texte attendues dans le print" ] )
...
TESTS = [
    ...
    (
        "[Ni(NH3)3(H2O)3]2+",
        [
            "IUPAC: triquatriaaminenickel(II)",
            "Oxidation: Ni (2+)",
            "Config: [Ar] 4s0 3d8",
            "Electrons: 20",
            "Geometry: Octahedral",
...
def test_analyse_compound(formula, expected_lines, capsys):
    """
    Exécute la fonction qui écrit dans la console, capture la sortie standard,
    et vérifie que chaque ligne attendue apparaît bien dans ce qui a été affiché.
    """
    # 1. On lance ta fonction
    analyse_compound(formula)

    # 2. On récupère tout ce qui a été affiché dans la console
    captured = capsys.readouterr()
    output_text = captured.out

    # 3. On vérifie que chaque morceau attendu est bien présent dans la sortie
    for line in expected_lines:
        assert line in output_text, (
            f"Ligne manquante ou fausse pour {formula} : attendu '{line}'"
        )
